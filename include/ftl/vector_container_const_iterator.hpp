...
class vector_container_const_iterator final : public const_iterator_interface<vector_container_const_iterator<Item>, Item, std::size_t> {
...
private:
  ...
  template<typename Callable>
  [[nodiscard]] auto filter_impl(Callable &&callable) const -> filter_iterator<vector_container_const_iterator<Item>, Callable> {
    return { *this, std::forward<Callable>(callable) };
  }
