...
private:
  ...
  [[nodiscard]] auto enumerate_impl() const -> enumerate_iterator<list_const_iterator<Item>> {
    return { *this };
  }
  ...
  auto preincrement_impl() const -> const list_const_iterator<Item> & {
    ++current_;
...
template<typename Item>
struct std::iterator_traits<ftl::list_const_iterator<Item>> {
  using difference_type = typename ftl::list_const_iterator<Item>::difference_type;
  using value_type = typename ftl::list_const_iterator<Item>::value_type;
  using pointer = typename ftl::list_const_iterator<Item>::pointer;
